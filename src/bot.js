...
class Falbot {
	...
	constructor() {
		...
		setInterval(
			() => {
				this.client.user.setActivity('/help | gCoin'), this.bankInterest(), this.lotteryDraw();
			},
			1000 * 60 * 5
		);
	...
	async rankPerks(old_rank, rank, interaction) {
		...
		if (old_rank != undefined) {
			if (old_rank.bankLimit < rank.bankLimit) {
				perks += this.getMessage(interaction, 'RANKUP_BANK', {
					GCOINS: format(rank.bankLimit - old_rank.bankLimit),
				});
				perks += '\n';
			}
			...
			if (old_rank.inventoryLimit < rank.inventoryLimit) {
				perks += this.getMessage(interaction, 'RANKUP_INVENTORY', {
					GCOINS: format(rank.inventoryLimit - old_rank.inventoryLimit),
				});
				perks += '\n';
			}
		...
		perks += `${this.getMessage(interaction, 'VOTE')}: ${format(rank.vote)} gCoins\n`;

		perks += `${this.getMessage(interaction, 'WORK')}: ${format(rank.work[0])}-${format(rank.work[1])} gCoins`;
	...
	createEmbed(color = 'Random') {
		return new EmbedBuilder().setColor(color).setFooter({ text: 'Wanna gCoins? Write /vote' });
	}
