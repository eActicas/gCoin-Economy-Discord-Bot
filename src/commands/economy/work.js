...
module.exports = {
	...
	data: new SlashCommandBuilder()
		...
		.setDescription('Go to work to earn gCoins')
		.setDescriptionLocalizations({
			'pt-BR': 'Vá trabalhar para ganhar gCoins',
			'es-ES': 'Ve a trabajar para ganar gCoins',
		})
	...
	execute: async ({ interaction, instance, member, user, database }) => {
		try {
			...
			player.gcoins += salary + bonus;

			var embed = instance
				.createEmbed(member.displayColor)
				.setTitle(
					instance.getMessage(interaction, 'WORK_TITLE', {
						GCOINS: format(salary + bonus),
					})
				)
