...
module.exports = {
	...
	execute: async ({ interaction, instance, member, subcommand, database }) => {
		...
		if (type == 'all') {
						...
						embed.addFields({
							...
							value:
								cheapestSellOrder.price == Infinity
									...
									: `${format(cheapestSellOrder.price)} gcoins`,
							inline: true,
						});
					}
			...
			//retrieve all buy orders and group them by price, putting the highest price first, and formatting the string like x gcoins - y availables
			...
			if (buyOrders != []) {
				...
				var formattedBuyOrders = Object.entries(groupedBuyOrders)
					...
					.map(
						(order) =>
							`${instance.getItemEmoji(itemKey)} **${format(order[0])} gCoins** - ${format(
								order[1]
							)} ${instance.getMessage(interaction, 'AVAILABLES')}`
					);
			}

			//retrieve all sell orders and group them by price, putting the lowest price first, and formatting the string like x gcoins (y orders)
			...
			while (
				...
				if (amount >= sellOrder.amount) {
					...
					userFile.gcoins -= sellOrder.price * sellOrder.amount;
			...
			if (userFile.gcoins < price * amount) {
				instance.editReply(interaction, {
					...
					Array.from({ length: numberOfPages }).map(async (_, i) => {
						...
						for (var order of ordersOnPage) {
							embed.addFields({
								...
								value: `${format(order.price)} gCoins - ${format(order.amount)} ${instance.getMessage(
									interaction,
									'AVAILABLES'
								)}`,
								inline: false,
			...
			if (listings == 'buy') {
				...
				userFile.gcoins += buyOrder.price * buyOrder.amount;
				await userFile.save();
