...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.setDescription('Challenge someone to a fight, whoever wins the fight gets all gCoins bet')
		.setDescriptionLocalizations({
			'pt-BR': 'Desafie outro usuário para uma luta, quem ganhar leva todos os gCoins apostados',
			'es-ES': 'Desafía a alguien a una pelea, quien gane la pelea se lleva todos los gCoins apostados',
		})
	...
	execute: async ({ guild, interaction, user, member, instance, database }) => {
		...
		try {
			...
			try {
				var bet = await specialArg(gcoins, author.gcoins);
			} catch {
				instance.editReply(interaction, {
			...
			if (author.gcoins >= bet && challengedFile.gcoins >= bet) {
					...
					if (collected.size === 0) {
						...
						const embed2 = instance.createEmbed(3066993).addFields(
							...
							},
							{
								...
								value: `${format(winnerFile.gcoins)} gCoins`,
							}
						);
