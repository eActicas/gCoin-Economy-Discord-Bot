...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.setNameLocalizations({
			'pt-BR': 'graça',
			'es-ES': 'carreradeperros',
		})
		.setDescription('Starts a dog race that other users can join')
		.setDescriptionLocalizations({
			'pt-BR': 'Inicia uma corrida de cães na qual outros usuários podem participar',
			'es-ES': 'Inicia una carrera de perros a la que otros usuarios pueden unirse',
		})
		...
		try {
			...
			if (player.gcoins >= bet) {
				...
				const embed = instance
					.createEmbed('#0099ff')
					.setDescription(
						instance.getMessage(interaction, 'DOGRACE_DESCRIPTION', {
							USER: user,
							BET: format(pot),
						}) + remainingTime
					)
				...
				collector.on('end', async (collected, reason) => {
					if (reason === 'refuse') {
						...
						embed.setDescription(
							instance.getMessage(interaction, 'DOGRACE_CANCEL', {
								USER: user,
								BET: format(pot),
							})
						);
						...
						while (true) {
							...
							for (let i = 0; i < path.length; i++) {
								frase += `${users[i]}\n:checkered_flag: ${path[i]}:dog2:\n\n`;
							}

							embed.setDescription(
								instance.getMessage(interaction, 'DOGRACE_DESCRIPTION2', {
									BET: format(pot),
								})
							);
						...
						embed.setDescription(
							instance.getMessage(interaction, 'DOGRACE_DESCRIPTION3', {
								BET: format(pot),
								USER: winner,
