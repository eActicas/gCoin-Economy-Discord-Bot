	...
	execute: async ({ guild, user, interaction, instance, database }) => {
		try {
			...
			if (type === 'rankup') {
				...
				if (rank.gcoinsToLevelUp === undefined) {
					await instance.editReply(interaction, {
						content: instance.getMessage(interaction, 'MAX_RANK', {
							USER: user,
						}),
					});
				} else if (player.gcoins < rank.gcoinsToLevelUp) {
					await instance.editReply(interaction, {
						content: instance.getMessage(interaction, 'NO_MONEY_RANK', {
							GCOINS: format(rank.gcoinsToLevelUp - player.gcoins),
						}),
					});
				} else {
					const new_rank = levels[player.rank];

					player.gcoins -= rank.gcoinsToLevelUp;
					player.rank += 1;
					...
					var embed = instance.createEmbed(1752220).addFields(
						{
							...
							value: instance.getMessage(interaction, 'RANKUP_SUCESS', {
								...
								GCOINS: format(rank.gcoinsToLevelUp),
							}),
						},
						{
				...
				if (levels[player.rank - 1].gcoinsToLevelUp === undefined) {
					await instance.editReply(interaction, {
						content: instance.getMessage(interaction, 'MAX_RANK', {
							USER: user,
						}),
				...
				var embed = instance
					...
					.addFields({
						name:
							...
							' - ' +
							format(levels[player.rank - 1].gcoinsToLevelUp) +
							' gCoins' +
				...
				for (var i = 0; i < quantity; i++) {
					if (levels[player.rank + i].gcoinsToLevelUp === undefined) {
						embed.addFields({
							name:
						...
						embed.addFields({
							name:
								instance.getMessage(interaction, String(player.rank + i + 1)) +
								' - ' +
								format(levels[player.rank + i].gcoinsToLevelUp) +
								' gCoins',
							value: await instance.rankPerks(levels[player.rank - 1 + i], levels[player.rank + i], interaction),
						});
					}
				...
				embed.setFooter({ text: 'gCoins [2024]' });
				...
				for (var i = 0; i < numEmbeds; i++) {
					...
					for (var j = 0; j < ranksChunk.length; j++) {
						if (levels[j + i * 20].gcoins === undefined) {
							ranks +=
					...
					embed
						...
						.setFooter({ text: 'Wanna gCoins? Write /vote' });
