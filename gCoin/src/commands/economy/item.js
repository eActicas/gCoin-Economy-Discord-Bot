...
module.exports = {
	...
	execute: async ({ interaction, instance, member, subcommand, args, database }) => {
		try {
			...
			if (itemJSON.value) {
				information += `:moneybag: ${instance.getMessage(interaction, 'COST')} **${format(itemJSON.value)} gCoins**`;
				const sellOrder = await database.market.getCheapestSellOrder(itemKey);
				if (sellOrder.price != Infinity) {
					information += `\n🛍️ ${instance.getMessage(interaction, 'BUY_FOR')} **${format(sellOrder.price)} gCoins**`;
				}
				const buyOrder = await database.market.getBestBuyOrder(itemKey);
				if (buyOrder.price != 0) {
					information += `\n🛒 ${instance.getMessage(interaction, 'SELL_FOR')} **${format(buyOrder.price)} gCoins**`;
				}
			} else {
